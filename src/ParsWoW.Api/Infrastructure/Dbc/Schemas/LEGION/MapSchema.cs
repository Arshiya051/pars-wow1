...
public sealed class LegionMapSchema : DbcSchemaBase<MapRecord>
{
    public override string FileName => "Map.dbc";
    public override MapRecord ProjectTyped(DbcRecord raw) => new()
    {
        Id = raw.GetInt32OrDefault(0), Directory = raw.GetStringOrDefault(1),
        InstanceType = raw.GetInt32OrDefault(2), Flags = raw.GetInt32OrDefault(3), MapName = raw.GetStringOrDefault(4)
    };
}
